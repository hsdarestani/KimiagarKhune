...
class Box(models.Model):
    ...
    def __str__(self):
        if self.name:
            return self.name
        lesson_name = self.lesson.name if self.lesson else "بدون درس"
        return f"{lesson_name} - {self.box_type.name}"
...
class DefaultEvent(models.Model):
    ...
    student = models.ForeignKey(
        ...
        related_name='default_events',
        verbose_name='دانش‌آموز',
    )
    name = models.CharField(max_length=100, verbose_name='عنوان ایونت')
    day_of_week = models.CharField(max_length=20, choices=DAY_CHOICES, verbose_name='روز هفته')
    start_time = models.TimeField(verbose_name='ساعت شروع')
    ...
    def __str__(self):
        return f"{self.student} - {self.name} - {self.day_of_week}"
...
class WeeklyReportDetail(models.Model):
    ...
    def __str__(self):
        return f"Box: {self.box} ({self.start_time} - {self.end_time})"
